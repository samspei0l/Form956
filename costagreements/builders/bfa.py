"""Builds the BFA (Binding Financial Agreement) Cost Agreement PDF.

BFA stands for **Binding Financial Agreement** -- a family-law agreement
between partners/spouses dividing property and finances (prepared under
Part VIIIA of the Family Law Act 1975), *not* a visa/migration product.
This is confirmed by the reference TS source's own default service
bullets ("Prepare your Binding Financial Agreement (BFA)", "Liaise with
your partner's lawyer", "Follow up until the BFA is executed") -- there is
no visa subclass, nomination, or DoHA/OMARA processing-time field
anywhere in this pathway.

Content (every clause, bullet, and disclosure paragraph) is transcribed
verbatim from winzoylegal_new's
src/features/bfaCostAgreement/buildBfaCostAgreementPdf.ts (schema mirrored
from that feature's types.ts) -- only the layout mechanism changed. See
...
Structurally, this is the simplest sibling that still carries a full
"General Terms of Business" page: like client_agreement.py it has no
"WHAT WE/YOU MUST DO", "REGULATORY COMPLIANCE" section, or Annexures
A/B/C (the TS source never imports the shared
``appendCostAgreementAnnexures`` helper, only ``appendAnnexureD``) -- but
unlike client_agreement.py it *does* append the full numbered-free
General Terms of Business page (mirroring buildBfaCostAgreementPdf.ts's
own ``terms`` array), then optionally Annexure D.

Structural decisions where the TS source was ambiguous:
  - ``REQUIRED_FIELDS``: types.ts marks almost every field as required
    (including serviceBullets, totalCost, paymentStage1Amount,
    paymentStage2Amount, marn, lpn) but, following the precedent set by
    every other sibling builder, only the fields load-bearing for page 1's
    body text to render sensibly are required here: date/our_ref/
    client_name/client_address/capacity, professional_fee/estimated_days/
    service_fee, rep_name. serviceBullets/totalCost/payment-stage amounts/
    marn/lpn all have sensible fallback text/defaults in ``_build_story``
    exactly like every sibling's own optional fields.
  - ``General Terms of Business`` heading text: the TS source's own
    ``terms`` array headings are *not* numbered ("Billing Arrangements",
    "Acceptance of Offer", ...) unlike jrp.py/skills_assessment_186.py's
    own local ``_GENERAL_TERMS``/``_JRP_TERMS`` lists (which prefix each
    heading "1 ...", "2 ..."). This port keeps the BFA source's own
    unnumbered heading text verbatim rather than imitating the numbering
    convention of unrelated siblings.
  - The "estimate cost" paragraph draws from ``data.total_cost`` (a
    client-supplied figure) verbatim, exactly as
    buildBfaCostAgreementPdf.ts does -- it is *not* the same number as the
    separately-computed Cost Summary table (``professional_fee +
    service_fee``), matching the TS source's own inline
    ``$${fmtAmt(data.totalCost || '')}`` text.
...
# --------------------------------------------------------------------- schema
@dataclass
class BfaCostAgreementData:
    """Payload shape for the BFA (Binding Financial Agreement) Cost
    Agreement, mirroring winzoylegal_new's bfaCostAgreement/types.ts
    ...
    # Payment schedule (fixed 2 stages -- no extra_stages in types.ts)
    ...
    marn: str = ""
    lpn: str = ""
    rep_signature_data: str | None = None       # base64 data URI -- see validate_bfa_cost_agreement()
    ...
    @classmethod
    def from_payload(cls, payload: dict) -> "BfaCostAgreementData":
...
def validate_bfa_cost_agreement(payload: dict) -> list[ValidationError]:
    """Validate a BFA Cost Agreement payload. Empty list = valid."""
...
def apply_bfa_normalisations(payload: dict) -> dict:
...
# --------------------------------------------------------------------- builder
_DEFAULT_BULLETS = [
    "Prepare your Binding Financial Agreement (BFA)",
    "Liaise with your partner's lawyer",
    "Follow up until the BFA is executed",
]

_STYLE_PAYMENT_NOTE = ParagraphStyle(
    "BFA_PaymentNote", fontName=L.FONT_REGULAR, fontSize=9.5, leading=13, textColor=L.BLACK,
)
_STYLE_AMT_CENTER = ParagraphStyle(
    "BFA_AmtCenter", fontName=L.FONT_REGULAR, fontSize=10, leading=13, alignment=1,
)


def build_bfa_cost_agreement(data: BfaCostAgreementData) -> bytes:
    ...
    def on_page(canvas, doc):
        ...
        chrome.draw_header(canvas, doc, doc_id, generated_at, "BFA")
...
def _make_doc_id(data: BfaCostAgreementData) -> str:
    seed = f"BFA|{data.our_ref}|{data.client_name}|{time.time()}"
    h = 0
    for ch in seed:
        h = (h * 31 + ord(ch)) & 0xFFFFFFFF
    hex_str = format(h, "08x").upper()
    return f"WZL-BFA-{hex_str[:4]}-{hex_str[4:8]}"
...
def _payment_schedule_table(stages: list[tuple[str, str]]) -> Table:
    ...
    buildBfaCostAgreementPdf.ts's fixed 2-row ``stages`` array (no
    extra_stages -- not present in types.ts)."""
    w1 = L.CONTENT_W * 0.70
    w2 = L.CONTENT_W * 0.30
...
def _build_story(data: BfaCostAgreementData, today_short: str) -> list:
    story: list = []

    # ═══════════════════════════════════════ PAGE 1 — Agreement details
    if data.translation_banner_text:
        banner_style = ParagraphStyle(
            "BFA_TransBanner", fontName=L.FONT_REGULAR, fontSize=8, leading=14,
    ...
    date_style = ParagraphStyle("BFA_DateLine", fontName=L.FONT_REGULAR, fontSize=10.5, alignment=2)
    ...
    days = data.estimated_days or "05"
    story.append(PT(
    ...
    disbursements_cost = sum_amounts(data.service_fee)
    ...
    stages: list[tuple[str, str]] = [
        ("1. On the day Cost Agreement is signed", f"${fmt_amt(data.payment_stage1_amount)}"),
        ("2. On the day the BFA is executed", f"${fmt_amt(data.payment_stage2_amount)}"),
    ]
...
# Headings transcribed verbatim from buildBfaCostAgreementPdf.ts's own
# ``terms`` array -- deliberately *not* numbered, unlike jrp.py's/
# skills_assessment_186.py's local terms lists (see module docstring).
_BFA_TERMS: list[tuple[str, str]] = [
    ("General Terms of Business", ""),
    (
...
def _general_terms() -> list:
    flows: list = []
    for i, (heading, body) in enumerate(_BFA_TERMS):
        is_title = i == 0
