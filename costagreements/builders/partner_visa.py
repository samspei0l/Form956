"""Builds the Partner Visa Cost Agreement PDF.

Content (every clause, bullet, and disclosure paragraph) is transcribed
verbatim from winzoylegal_new's
src/features/partnerVisaCostAgreement/buildPartnerVisaCostAgreementPdf.ts
...
This type differs from the other ports in a few structural ways (all
mirrored from the TS source, not invented here):
  1. Page 1 has its own unlabelled intro paragraph ("This document,
     together with our General Terms of Business, sets out the terms of
     our offer...") right after the parties table -- ART/JRP don't have
     this; General does.
  2. The numbered sections skip straight from C to F (no D/E): "A. Scope
     of Work", "B. Disbursements and Internal Expenses", "C. Estimate of
     Professional Fees and Internal Expenses" (a 2-row line-item table --
     Professional Cost / Disbursement Lodgement Fee -- distinct from the
     derived Cost Summary table that follows it), then an unlabelled
     "Variables" subsection, "F. Payment Schedule for Cost and
     Disbursement" (a 3-stage + extra-stages table, like ART/JRP), "G.
  ...
  4. "WHAT WE/YOU MUST DO" and "REGULATORY COMPLIANCE AND APPLICABLE LAW"
     ...
     "No Guarantee of Review Outcome"). "General Terms of Business"
     clauses 9 (Termination by Us), 10 (Termination by You) and 12
     (Privacy) have their own, shorter bodies here -- the TS source drops
     the ART/JRP-only "Court directions" caveat from 9(b), the litigation
     cancellation-fee caveat from 10, and the trust-account Rule 47
     sentence from 12. All other clauses (1-8, 11, 13-15) are identical
     wording to ART/JRP.
  5. Calls the same appendCostAgreementAnnexures-equivalent (Annexures
  ...
  6. Uses SigMetaState (costagreements/sigmeta.py) -- the TS source calls
     ``pdfDoc.setSubject('SIGMETA:' + ...)`` itself, and every sibling
     builder that already ported that behaviour (client_agreement, art,
     jrp, skills_assessment_only) does the same here.

Structural decisions where the TS source was ambiguous or silent:
  - ``REQUIRED_FIELDS``: types.ts marks 21 fields as non-optional
    ...
    (ART, JRP) excludes booleans, derived/echoed totals, and marn/lpn
    (only one of which is ever relevant, depending on capacity) from its
    own REQUIRED_FIELDS even where the TS type marks them non-optional.
    This port follows that same precedent: only the fields that are
    actually load-bearing for page 1's body text to render sensibly are
    required (date/our_ref/client_name/client_address/capacity,
    visa_subclass/professional_fee/disbursement_lodgement_fee/
    processing_time, rep_name). Payment-stage labels/amounts, marn, lpn,
    total_cost, and lodgement_uses_client_card all have sensible
    defaults ("" / False / fmt_amt("") == "0.00") exactly like ART's and
    JRP's equivalent optional fields.
  - ``service_bullets``: types.ts types this as a plain ``string`` (not
    ``string[]`` like ART's/JRP's ``serviceBullets``), and the TS builder
    never actually reads ``data.serviceBullets`` anywhere in its drawing
    code -- it's a vestigial field on the payload shape. Kept here as a
    plain ``str`` field for schema parity; intentionally unused in
    ``_build_story``.
...
# --------------------------------------------------------------------- schema
@dataclass
class PartnerVisaCostAgreementData:
    """Payload shape for the Partner Visa Cost Agreement, mirroring
    winzoylegal_new's partnerVisaCostAgreement/types.ts field-for-field
    ...
    # Scope of work / fees
    visa_subclass: str  # e.g. "820", "309"
    professional_fee: str
    disbursement_lodgement_fee: str
    processing_time: str  # e.g. "15 - 23 months"

    rep_name: str

    service_bullets: str = ""  # typed as a plain string in types.ts; unused in rendering -- see module docstring
    ...
    marn: str = ""
    lpn: str = ""
    rep_signature_data: str | None = None       # base64 data URI -- see validate_partner_visa_cost_agreement()
    ...
    @classmethod
    def from_payload(cls, payload: dict) -> "PartnerVisaCostAgreementData":
        ...
        return cls(
            ...
            disbursement_lodgement_fee=str(payload.get("disbursement_lodgement_fee") or ""),
            processing_time=str(payload.get("processing_time") or ""),
...
REQUIRED_FIELDS = frozenset({
    ...
    "visa_subclass", "professional_fee", "disbursement_lodgement_fee", "processing_time",
    "rep_name",
})

VALID_CAPACITIES = frozenset({"solicitor", "rma"})
MONEY_FIELDS = frozenset({
    "professional_fee", "disbursement_lodgement_fee",
...
def validate_partner_visa_cost_agreement(payload: dict) -> list[ValidationError]:
    """Validate a Partner Visa Cost Agreement payload. Empty list = valid."""
...
def apply_partner_visa_normalisations(payload: dict) -> dict:
...
_STYLE_ITALIC_SMALL = ParagraphStyle(
    "PV_ItalicSmall", fontName=L.FONT_ITALIC, fontSize=9.5, leading=12.5, textColor=L.BLACK,
)


def build_partner_visa_cost_agreement(data: PartnerVisaCostAgreementData) -> bytes:
    ...
    def on_page(canvas, doc):
        ...
        chrome.draw_header(canvas, doc, doc_id, generated_at, "PARTNER VISA")
...
def _make_doc_id(data: PartnerVisaCostAgreementData) -> str:
    seed = f"PV|{data.our_ref}|{data.client_name}|{time.time()}"
    h = 0
    for ch in seed:
        h = (h * 31 + ord(ch)) & 0xFFFFFFFF
    hex_str = format(h, "08x").upper()
    return f"WZL-PV-{hex_str[:4]}-{hex_str[4:8]}"
...
def _estimate_table(professional_fee_text: str, lodgement_text: str) -> Table:
    """The 'C. Estimate of Professional Fees and Internal Expenses' 2-row
    line-item table -- matches buildPartnerVisaCostAgreementPdf.ts's own
    drawTableRow() calls (blank/'Amount' shaded header, centred amount
    column). Distinct from the derived Cost Summary table drawn straight
    after it."""
    w1 = L.CONTENT_W * 0.55
    w2 = L.CONTENT_W * 0.45
    head_amt_style = ParagraphStyle("PV_EstimateHeadAmt", fontName=L.FONT_BOLD, fontSize=10, alignment=1)
    amt_style = ParagraphStyle("PV_EstimateAmt", fontName=L.FONT_REGULAR, fontSize=10, alignment=1)
    data = [
        ["", PT("Amount", head_amt_style)],
        [PT("1. Professional Cost", L.STYLE_BODY), P(esc(professional_fee_text), amt_style)],
        [PT("2. Disbursement Lodgement Fee", L.STYLE_BODY), P(esc(lodgement_text), amt_style)],
    ]
...
def _payment_schedule_table(stages: list[tuple[str, str]]) -> Table:
    ...
    buildPartnerVisaCostAgreementPdf.ts's 'F. Payment Schedule for Cost
...
def _build_story(data: PartnerVisaCostAgreementData, today_short: str) -> list:
    story: list = []

    # ═══════════════════════════════════════ PAGE 1 — Agreement details
    if data.translation_banner_text:
        banner_style = ParagraphStyle(
            "PV_TransBanner", fontName=L.FONT_REGULAR, fontSize=8, leading=14,
    ...
    date_style = ParagraphStyle("PV_DateLine", fontName=L.FONT_REGULAR, fontSize=10.5, alignment=2)
    ...
    story.append(Spacer(1, 4))
    story.append(PT(
        f"1. You have instructed us to prepare and act for your Temporary Partner "
        f"Visa (Subclass {data.visa_subclass or '___'}).",
        L.STYLE_BODY_SMALL,
    ))
    story.append(Spacer(1, 10))

    story.append(P("Professional Fees", L.STYLE_H2))
    story.append(Spacer(1, 4))
    story.append(PT(
    ...
    story.append(P("B. Disbursements and Internal Expenses", L.STYLE_H2))
    story.append(Spacer(1, 4))
    story.append(PT(
        "You may incur disbursements. Disbursements may include Personal Identifier "
        "Fees, Police Check Fees, Immigration Health Check Fees, Fees in registering "
        "and obtaining marriage certificate, and Fees in obtaining proof of Divorce. "
        "We will notify you of these Disbursements and you are required to pay them "
        "accordingly or instruct us to assist you with the payment directly.",
        L.STYLE_BODY,
    ))
    story.append(Spacer(1, 12))

    # ── C. Estimate of Professional Fees and Internal Expenses ──
    story.append(P("C. Estimate of Professional Fees and Internal Expenses", L.STYLE_H2))
    story.append(Spacer(1, 4))
    professional_fee_text = f"${fmt_amt(data.professional_fee)} inclusive of GST"
    lodgement_text = f"${fmt_amt(data.disbursement_lodgement_fee)} + 1.4% visa card fee"
    if data.lodgement_uses_client_card:
        lodgement_text += " (client card)"
    story.append(_estimate_table(professional_fee_text, lodgement_text))
    story.append(Spacer(1, 10))
    ...
    story.append(Spacer(1, 4))
    story.append(PT(
        f"Based on current processing times, you can expect your subclass "
        f"{data.visa_subclass or '___'} visa to be finalized in about "
        f"{data.processing_time or 'Depends upon DoHA Global Processing Times'} "
        "from the date of the formal application. In ordinary processing time "
        "frame, as we cannot control the works of the Department of Immigration & "
        "Citizenship, the above time frame is only an estimate of time, not definitive.",
        L.STYLE_BODY_SMALL,
    ))
    story.append(Spacer(1, 8))
    story.append(PT(
    ...
    story.append(Spacer(1, 4))
    story.append(P(bulleted_html([
        "a) The applicant's past migration history;",
        ...
        "f) The complete evidence being made or submitted at the time of the "
...
def _what_we_you_must_do() -> list:
    ...
    ila_head_style = ParagraphStyle("PV_ILA", fontName=L.FONT_BOLD, fontSize=9, textColor=L.NAVY)
    extra_right = [
