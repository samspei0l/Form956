"""Builds the Skills Assessment 186 Cost Agreement PDF -- Skills Assessment
+ Employer Nomination Scheme (Subclass 186 Direct Entry) visa pathway,
bundled into a single agreement.

Content (every clause, bullet, and disclosure paragraph) is transcribed
verbatim from winzoylegal_new's
src/features/skillsAssessment186CostAgreement/buildSkillsAssessment186CostAgreementPdf.ts
...
NOT THE SAME PATHWAY AS skills_assessment.py
----------------------------------------------
``skills_assessment.py`` ports the *state-nomination* pathway (Skills
Assessment + EOI + state nomination + skilled visa, subclass 491/190/189):
its schema carries ``visa_subclass``/``nomination_state``/
``state_nomination_fee`` and a 4-stage payment schedule. This module was
diffed (via ``diff``) against that sibling's own
``skillsAssessmentCostAgreement/types.ts`` and is structurally distinct --
it is the *employer-nomination* pathway (Skills Assessment + Employer
Nomination Scheme Subclass 186 Direct Entry) and carries:
  - no ``visa_subclass`` / ``nomination_state`` at all -- the visa type is
    fixed text ("Employer Nomination Scheme (Subclass 186 Direct Entry)
    Visa"), not a client-selectable field;
  - ``nomination_fee`` (a flat ENS nomination lodgement fee) instead of
    ``state_nomination_fee``;
  - ``saf_levy_small`` / ``saf_levy_large`` (the either/or Skilling
    Australians Fund levy, keyed to the sponsoring employer's annual
    turnover being under or over $10m) -- entirely absent from the
    state-nomination pathway, which has no employer-turnover concept;
  - ``sponsorship_processing_time`` (how long DoHA takes to approve the
    *business sponsorship* application before nomination can be lodged) in
    place of ``skill_assessment_processing_time`` -- 186 Direct Entry does
    not have a distinct "priority processing" assessment-authority
    concept, so there is no ``priority_processing_fee`` field either;
  - only a 3-stage default payment schedule (Skills Assessment application
    / Nomination lodgement / Visa application lodgement) rather than the
    4-stage EOI/nomination/visa split of the state-nomination pathway.
  NOTE ON employer_name: the porting brief for this module anticipated an
  ``employer_name`` (or similarly employer-identifying) field given the
  186 pathway names an actual sponsoring employer. No such field exists in
  the real ``skillsAssessment186CostAgreement/types.ts`` -- the sponsoring
  employer is referenced only implicitly ("once the business sponsorship
  application is approved...") and never named on the document itself.
  This port does not invent one; the schema below is a field-for-field
  mirror of the real TS type, nothing more.
skills_assessment.py is still the closer sibling for the overall
structure (full pathway: assessment + follow-on visa, full signature
block, letter-lettered A-G sections) -- this module reuses that shape,
just swapping in the 186-specific field names/defaults/copy above and the
JRP-style 3-row (not 4-row) payment schedule table pattern.

Structural decisions where the TS source was ambiguous:
  - ``REQUIRED_FIELDS``: types.ts marks almost every field as required
    (including anzscoCode, safLevySmall, safLevyLarge, marn, lpn,
    totalCost, lodgementUsesClientCard, and all three payment-stage
    label/amount pairs) but, following the precedent set by
    skills_assessment.py and skilled_visa.py (the closer siblings), only
    the fields load-bearing for page 1's body text to render sensibly are
    required here: date/our_ref/client_name/client_address/capacity,
    occupation/assessing_authority, professional_fee/nomination_fee/
    visa_application_fee, sponsorship_processing_time/
    nomination_processing_time/visa_processing_time, rep_name.
    anzsco_code, saf_levy_small, saf_levy_large,
    lodgement_uses_client_card, total_cost, marn, lpn, and all three
  ...
  - SAF levy is deliberately *not* summed into the Cost Summary's
...
# --------------------------------------------------------------------- schema
@dataclass
class SkillsAssessment186CostAgreementData:
    """Payload shape for the Skills Assessment 186 Cost Agreement,
    mirroring winzoylegal_new's skillsAssessment186CostAgreement/types.ts
    field-for-field (snake_case to match this API's existing JSON
    convention)."""
    ...
    occupation: str               # e.g. "Cabinet Maker"
    assessing_authority: str      # e.g. "ATTC", "TRA", "Vetassess"
    ...
    anzsco_code: str = ""             # e.g. "394111"
    saf_levy_small: str = ""          # e.g. "3,000" (turnover under $10m)
    saf_levy_large: str = ""          # e.g. "5,000" (turnover $10m or more)
    ...
    marn: str = ""
    lpn: str = ""
    rep_signature_data: str | None = None       # base64 data URI -- see validate_skills_assessment_186_cost_agreement()
    ...
    @classmethod
    def from_payload(cls, payload: dict) -> "SkillsAssessment186CostAgreementData":
        ...
        return cls(
            ...
            saf_levy_small=str(payload.get("saf_levy_small") or ""),
            saf_levy_large=str(payload.get("saf_levy_large") or ""),
...
REQUIRED_FIELDS = frozenset({
    ...
    "professional_fee", "nomination_fee", "visa_application_fee",
...
def validate_skills_assessment_186_cost_agreement(payload: dict) -> list[ValidationError]:
    """Validate a Skills Assessment 186 Cost Agreement payload. Empty list = valid."""
...
def apply_skills_assessment_186_normalisations(payload: dict) -> dict:
...
_STYLE_ITALIC_SMALL = ParagraphStyle(
    "SA186_ItalicSmall", fontName=L.FONT_ITALIC, fontSize=9.5, leading=12.5, textColor=L.BLACK,
)
_STYLE_DISB_CELL = ParagraphStyle(
    "SA186_DisbCell", fontName=L.FONT_REGULAR, fontSize=8.5, leading=11, textColor=L.BLACK,
)


def build_skills_assessment_186_cost_agreement(data: SkillsAssessment186CostAgreementData) -> bytes:
    ...
    def on_page(canvas, doc):
        ...
        chrome.draw_header(canvas, doc, doc_id, generated_at, "SKILLS ASSESSMENT + SUBCLASS 186")

    template = PageTemplate(id="main", frames=[frame], onPage=on_page)
    doc = BaseDocTemplate(
        buf, pagesize=(L.PAGE_W, L.PAGE_H), pageTemplates=[template],
        title="Costs Disclosure and Costs Agreement (Skills Assessment + Subclass 186)", author="Winzoy Legal",
...
def _make_doc_id(data: SkillsAssessment186CostAgreementData) -> str:
    seed = f"SA186|{data.our_ref}|{data.client_name}|{time.time()}"
    h = 0
    for ch in seed:
        h = (h * 31 + ord(ch)) & 0xFFFFFFFF
    hex_str = format(h, "08x").upper()
    return f"WZL-SA186-{hex_str[:4]}-{hex_str[4:8]}"
...
def _disb_bullets_html(data: "SkillsAssessment186CostAgreementData", vac_surcharged: float) -> str:
    ...
    an un-bulleted 'or $X (turnover $10m or more).' continuation), not
    four uniformly-bulleted lines."""
    nomination_fee_text = fmt_amt(data.nomination_fee or "540")
    saf_small = data.saf_levy_small or "3,000"
...
def _estimate_table(data: SkillsAssessment186CostAgreementData, vac_surcharged: float) -> Table:
    ...
    w1 = L.CONTENT_W * 0.50
    w2 = L.CONTENT_W * 0.50
    head_amt_style = ParagraphStyle("SA186_EstimateHeadAmt", fontName=L.FONT_BOLD, fontSize=10, alignment=1)
    amt_style = ParagraphStyle("SA186_EstimateAmt", fontName=L.FONT_REGULAR, fontSize=10, alignment=1)
...
def _payment_schedule_table(stages: list[tuple[str, str]]) -> Table:
    ...
    buildSkillsAssessment186CostAgreementPdf.ts's 'E. Payment Schedule for
    Cost and Disbursement' rows (3 default stages + optional extras)."""
    w1 = L.CONTENT_W * 0.70
    w2 = L.CONTENT_W * 0.30
    amt_style = ParagraphStyle("SA186_PaySchedAmt", fontName=L.FONT_REGULAR, fontSize=10.5, alignment=1)
...
def _build_payment_stages(data: SkillsAssessment186CostAgreementData) -> list[tuple[str, str]]:
    stages = [
        (data.payment_stage1_label or "1. Skills Assessment application", f"${fmt_amt(data.payment_stage1_amount)}"),
        (data.payment_stage2_label or "2. Nomination lodgement", f"${fmt_amt(data.payment_stage2_amount)}"),
        (data.payment_stage3_label or "3. Visa application lodgement", f"${fmt_amt(data.payment_stage3_amount)}"),
    ]
...
def _build_story(data: SkillsAssessment186CostAgreementData, today_short: str) -> list:
    story: list = []

    # ═══════════════════════════════════════ PAGE 1 — Agreement details
    if data.translation_banner_text:
        banner_style = ParagraphStyle(
            "SA186_TransBanner", fontName=L.FONT_REGULAR, fontSize=8, leading=14,
    ...
    date_style = ParagraphStyle("SA186_DateLine", fontName=L.FONT_REGULAR, fontSize=10.5, alignment=2)
    ...
    story.append(PT(
        f"1.1 Lodge your application for your Skills Assessment as a {occ_text} to "
        f"{data.assessing_authority or 'ATTC'}.",
        L.STYLE_BODY_SMALL,
    ))
    story.append(Spacer(1, 2))
    story.append(PT(
        "1.2 Lodge your application for your Employer Nomination Scheme (Subclass "
...
def _what_we_you_must_do() -> list:
    wmd = [
        ...
        "Lodge the Skills Assessment, nomination and Subclass 186 visa applications "
        "to the relevant Assessing Authority and the Department of Home Affairs, "
        "with supporting documentation, liaise with you as to necessary application, "
    ...
    ila_head_style = ParagraphStyle("SA186_ILA", fontName=L.FONT_BOLD, fontSize=9, textColor=L.NAVY)
    extra_right = [
